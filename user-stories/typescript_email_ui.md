...
engineer | UI tests to run against the in-memory `FakeProvider` from `typescript_mail_provider.md` and `FakeIntelligence` from `typescript_mail_intelligence.md`, seeded with fixture data using fake addresses | tests are deterministic, reproducible, and touch no bridge, network, or AI API
