...
## Input
Runtime inputs are the plug-in instances passed to `register` at the composition root, the model objects handed to dispatch calls, and the injected `PluginSettings` storage. In tests, inputs are `FakePlugin`/`ThrowingPlugin` fixtures and in-memory settings.

## Output Schema
name, type, format (optional)
`MailPlugin`, interface, `{id: string, name: string, version: string, apiVersion: number, contributes(): ExtensionPoint[], messageView?, composeAction?, threadAction?, settingsPanel?}`
`ExtensionPoint`, union, `'messageView' | 'composeAction' | 'threadAction' | 'settingsPanel'`
`ViewContribution`, object, `{pluginId: string, title: string, bodyText: string}`
`PluginAction`, object, `{pluginId: string, label: string, run(): Promise<void>}`
`SettingsContribution`, object, `{pluginId: string, title: string, fields: {key: string, label: string, value: string}[]}`
`PluginHost.list()` item, object, `{id: string, name: string, version: string, enabled: boolean, contributes: ExtensionPoint[], error?: string}`
