...
engineer | AI failures degrade, never block: every core mail flow (list, read, tag manually, compose, send) works when `MailIntelligence` rejects, with AI affordances showing error copy keyed off `MailIntelligenceError.code` and a retry | AI is fundamental to the experience but a rate limit or missing key never locks me out of my mail
engineer | app startup to construct providers and the intelligence backend (Gmail via its config, `ClaudeIntelligence` via its key) in one composition-root module, `src/main.tsx`-adjacent, the only file allowed to import concrete provider or intelligence classes | the proxy wiring lives in exactly one place
engineer | the Gmail bridge `baseUrl` and the Anthropic key read from a single config module honoring `VITE_BRIDGE_URL` (default `http://127.0.0.1:8765`) and `VITE_ANTHROPIC_API_KEY` env overrides | the Android emulator (`10.0.2.2`), a physical device, and the AI backend are each configured without code edits
