...
## Input
None at runtime — fixtures and options are passed to the constructor; nothing is read from network, disk, or environment.

## Output Schema
Exactly the shared contract types defined in `typescript_mail_provider.md` (`Tag`, `ThreadSummary`, `Message`, `ThreadPage`, `ProviderCapabilities`, `SendResult`, and `MailProviderError`; `send` consumes a `Draft`). `FakeProviderFixtures` and `FakeProviderOptions` are the only types of its own, both constructor-only.
